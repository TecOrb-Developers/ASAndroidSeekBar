...
import com.seekbardetail.Utils.Formate;
...
public class DisplayingSeekBar extends View {
    ...
    private SeekBarProperty mSeekParams;
    //seek bar
    ...
    //indicator
    private DisplayValue mIndicator;
    ...
    public DisplayingSeekBar(Context context) {
        this(context, null);
    }

    public DisplayingSeekBar(Context context, AttributeSet attrs) {
        super(context, attrs);
        this.mContext = context;
        initAttrs(mContext, attrs);
        initParams();
    }
    public DisplayingSeekBar(Context context, AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
        this.mContext = context;
        initAttrs(mContext, attrs);
        initParams();
    }
    DisplayingSeekBar(Builder builder) {
        super(builder.context);
        this.mContext = builder.context;
        int defaultPadding = Size.dp2px(mContext, 16);
    ...
    private void initAttrs(Context context, AttributeSet attrs) {
        ...
        TypedArray ta = context.obtainStyledAttributes(attrs, R.styleable.indexSeekbar);
        //seekBar
        mMax = ta.getFloat(R.styleable.indexSeekbar_isb_max, builder.max);
        mMin = ta.getFloat(R.styleable.indexSeekbar_isb_min, builder.min);
        mProgress = ta.getFloat(R.styleable.indexSeekbar_isb_progress, builder.progress);
        mIsFloatProgress = ta.getBoolean(R.styleable.indexSeekbar_isb_progress_value_float, builder.progressValueFloat);
        mUserSeekable = ta.getBoolean(R.styleable.indexSeekbar_isb_user_seekable, builder.userSeekable);
        mClearPadding = ta.getBoolean(R.styleable.indexSeekbar_isb_clear_default_padding, builder.clearPadding);
        mOnlyThumbDraggable = ta.getBoolean(R.styleable.indexSeekbar_isb_only_thumb_draggable, builder.onlyThumbDraggable);
        mSeekSmoothly = ta.getBoolean(R.styleable.indexSeekbar_isb_seek_smoothly, builder.seekSmoothly);
        mR2L = ta.getBoolean(R.styleable.indexSeekbar_isb_r2l, builder.r2l);
        //track
        mBackgroundTrackSize = ta.getDimensionPixelSize(R.styleable.indexSeekbar_isb_track_background_size, builder.trackBackgroundSize);
        mProgressTrackSize = ta.getDimensionPixelSize(R.styleable.indexSeekbar_isb_track_progress_size, builder.trackProgressSize);
        mBackgroundTrackColor = ta.getColor(R.styleable.indexSeekbar_isb_track_background_color, builder.trackBackgroundColor);
        mProgressTrackColor = ta.getColor(R.styleable.indexSeekbar_isb_track_progress_color, builder.trackProgressColor);
        mTrackRoundedCorners = ta.getBoolean(R.styleable.indexSeekbar_isb_track_rounded_corners, builder.trackRoundedCorners);
        //thumb
        mThumbSize = ta.getDimensionPixelSize(R.styleable.indexSeekbar_isb_thumb_size, builder.thumbSize);
        mThumbDrawable = ta.getDrawable(R.styleable.indexSeekbar_isb_thumb_drawable);
        mAdjustAuto = ta.getBoolean(R.styleable.indexSeekbar_isb_thumb_adjust_auto, true);
        initThumbColor(ta.getColorStateList(R.styleable.indexSeekbar_isb_thumb_color), builder.thumbColor);
        //thumb text
        mShowThumbText = ta.getBoolean(R.styleable.indexSeekbar_isb_show_thumb_text, builder.showThumbText);
        mThumbTextColor = ta.getColor(R.styleable.indexSeekbar_isb_thumb_text_color, builder.thumbTextColor);
        //tickMarks
        mTicksCount = ta.getInt(R.styleable.indexSeekbar_isb_ticks_count, builder.tickCount);
        mShowTickMarksType = ta.getInt(R.styleable.indexSeekbar_isb_show_tick_marks_type, builder.showTickMarksType);
        mTickMarksSize = ta.getDimensionPixelSize(R.styleable.indexSeekbar_isb_tick_marks_size, builder.tickMarksSize);
        initTickMarksColor(ta.getColorStateList(R.styleable.indexSeekbar_isb_tick_marks_color), builder.tickMarksColor);
        mTickMarksDrawable = ta.getDrawable(R.styleable.indexSeekbar_isb_tick_marks_drawable);
        mTickMarksSweptHide = ta.getBoolean(R.styleable.indexSeekbar_isb_tick_marks_swept_hide, builder.tickMarksSweptHide);
        mTickMarksEndsHide = ta.getBoolean(R.styleable.indexSeekbar_isb_tick_marks_ends_hide, builder.tickMarksEndsHide);
        //tickTexts
        mShowTickText = ta.getBoolean(R.styleable.indexSeekbar_isb_show_tick_texts, builder.showTickText);
        mTickTextsSize = ta.getDimensionPixelSize(R.styleable.indexSeekbar_isb_tick_texts_size, builder.tickTextsSize);
        initTickTextsColor(ta.getColorStateList(R.styleable.indexSeekbar_isb_tick_texts_color), builder.tickTextsColor);
        mTickTextsCustomArray = ta.getTextArray(R.styleable.indexSeekbar_isb_tick_texts_array);
        initTextsTypeface(ta.getInt(R.styleable.indexSeekbar_isb_tick_texts_typeface, -1), builder.tickTextsTypeFace);
        //indicator
        mShowIndicatorType = ta.getInt(R.styleable.indexSeekbar_isb_show_indicator, builder.showIndicatorType);
        mIndicatorColor = ta.getColor(R.styleable.indexSeekbar_isb_indicator_color, builder.indicatorColor);
        mIndicatorTextSize = ta.getDimensionPixelSize(R.styleable.indexSeekbar_isb_indicator_text_size, builder.indicatorTextSize);
        mIndicatorTextColor = ta.getColor(R.styleable.indexSeekbar_isb_indicator_text_color, builder.indicatorTextColor);
        int indicatorContentViewId = ta.getResourceId(R.styleable.indexSeekbar_isb_indicator_content_layout, 0);
        if (indicatorContentViewId > 0) {
            mIndicatorContentView = View.inflate(mContext, indicatorContentViewId, null);
        }
        int indicatorTopContentLayoutId = ta.getResourceId(R.styleable.indexSeekbar_isb_indicator_top_content_layout, 0);
    ...
    private void initParams() {
        ...
        if (mThumbDrawable == null) {
            ...
            mThumbRadius = Math.min(Size.dp2px(mContext, THUMB_MAX_WIDTH), mThumbSize) / 2.0f;
            mThumbTouchRadius = mThumbRadius;
        }
        if (mTickMarksDrawable == null) {
            mTickRadius = mTickMarksSize / 2.0f;
        } else {
            mTickRadius = Math.min(Size.dp2px(mContext, THUMB_MAX_WIDTH), mTickMarksSize) / 2.0f;
        }
    ...
    private void initDefaultPadding() {
        if (!mClearPadding) {
            int normalPadding = Size.dp2px(mContext, 16);
            if (getPaddingLeft() == 0) {
    ...
    private void measureTickTextsBonds() {
        if (needDrawText()) {
            ...
            mTickTextsHeight = mRect.height() + Size.dp2px(mContext, 3);//with the gap(3dp) between tickTexts and track.
        }
    }
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        setMeasuredDimension(resolveSize(Size.dp2px(mContext, 170), widthMeasureSpec), height + mTickTextsHeight);
        initSeekBarInfo();
    ...
    private void refreshSeekBarLocation() {
        ...
        if (needDrawText()) {
            mTextPaint.getTextBounds("j", 0, 1, mRect);
            mTickTextY = mPaddingTop + mCustomDrawableMaxHeight + Math.round(mRect.height() - mTextPaint.descent()) + Size.dp2px(mContext, 3);
    ...
    private void drawTickMarks(Canvas canvas) {
        ...
        for (int i = 0; i < mTickMarksX.length; i++) {
            ...
            if (mShowTickMarksType == TickMarkType.OVAL) {
                ...
                int rectWidth = Size.dp2px(mContext, 1);
    ...
    private Bitmap getDrawBitmap(Drawable drawable, boolean isThumb) {
        ...
        int width;
        int height;
        int maxRange = Size.dp2px(mContext, THUMB_MAX_WIDTH);
        int intrinsicWidth = drawable.getIntrinsicWidth();
    ...
    private boolean isTouchSeekBar(float mX, float mY) {
        if (mFaultTolerance == -1) {
            mFaultTolerance = Size.dp2px(mContext, 5);
        }
    ...
    private void initIndicatorContentView() {
        if (mShowIndicatorType == SeekBarIndicatorType.NONE) {
            return;
        }
        if (mIndicator == null) {
            mIndicator = new DisplayValue(mContext,
                    this,
                    mIndicatorColor,
                    mShowIndicatorType,
                    mIndicatorTextSize,
    ...
    private String getProgressString(float progress) {
        if (mIsFloatProgress) {
            return Formate.fastFormat(progress, mScale);
        } else {
    ...
    private SeekBarProperty collectParams(boolean formUser) {
        if (mSeekParams == null) {
            mSeekParams = new SeekBarProperty(this);
        }
    ...
    public DisplayValue getIndicator() {
        return mIndicator;
    }
